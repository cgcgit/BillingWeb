...
public class FeeTypeController extends BasicHistoricWithLists<CtFeeType>
	...
	public void init() {
		...
		if (this.getSelectedDataList() == null) {
			this.setSelectedDataList(new ArrayList<CtFeeType>());
		}
		...
		if (this.getSelectedHistoricData() == null) {
			this.setSelectedHistoricData(new CtFeeType());
		}
	...
	public void loadDataList() {
		...
		if (this.isHistoricSearchDataCriteria()) {
			this.setDataList(feeTypeEJB.findAllData());
	...
	public void pushShowDetailRowButton() {
		...
		this.setSelectedData((CtFeeType) dataTable.getRowData());

		if (this.getSelectedData() == null || this.getSelectedData().getFeeTypeId() == null
				|| this.getSelectedData().getFeeTypeId() == 0) {
	...
	public void pushAddNewRowButton() {
		...
		try {
			...
			this.setSelectedHistoricData((CtFeeType) Utilities.deepClone(currentData));
		...
		try {
			if (this.objectValidation(this.getNewData())) {
				if (this.isFromAddingRow()) {
					...
					// create a new object
					feeTypeEJB.insertData(this.getNewData());
					CtFeeType object = feeTypeEJB.findDataBySearchDateAndCode(this.getNewData().getStartDate(),
							this.getNewData().getCode());
	...
	public boolean splitRecords(DataTable dataTable, int row, Object newRow) {
		...
		CtFeeType newObject, originalPreviousDataRow, previousDataRow, subsequentDataRow;

		try {

			newObject = (CtFeeType) newRow;
				...
				if (!error) {
					...
					if (row != totalRows - 1) {
						...
						if (previousDataRow.getEndDate().isEqual(newObject.getStartDate().minusDays(1))
							...
							feeTypeEJB.insertNewHistoricDataRecord(newObject);
					...
					if (!coverGap) {
						...
						if ((newObject.getEndDate().isEqual(previousDataRow.getStartDate()))
							...
							if (newObject.getStartDate().isEqual(previousDataRow.getStartDate())) {
								...
								previousDataRow = (CtFeeType) Utilities.deepClone(newObject);
