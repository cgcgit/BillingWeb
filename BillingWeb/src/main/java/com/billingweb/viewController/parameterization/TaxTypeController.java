...
import com.billingweb.ejb.parameterization.TaxTypeEJBLocal;
 ...
 *
 */
public class TaxTypeController extends SimpleTableBasicClass implements Serializable, IGeneralController {

	/**
	 * 
	 */
	private static final long serialVersionUID = -2346822268417079274L;

	Logger logger = (Logger) LogManager.getLogger(TaxTypeController.class);

	String DATA_TABLE_ID = "form:" + TaxTypeController.uiValues.getString("dataTableID");
	String NEW_PANEL_DATA_ID = "form:" + TaxTypeController.uiValues.getString("newPanelDataID");
	 ...
	 */
	private List<PtTaxType> dataList;

	/**
	 * Application level filtered data list
	 */
	private List<PtTaxType> filteredDataList;
	 ...
	 * @return the dataList
	 */
	public List<PtTaxType> getDataList() {
		return dataList;
	}
	 ...
	 */
	public void setDataList(List<PtTaxType> dataList) {
	 ...
	 */
	public List<PtTaxType> getFilteredDataList() {
	 ...
	 */
	public void setFilteredDataList(List<PtTaxType> filteredDataList) {
	 ...
	 */
	public PtTaxType getBackupData() {
		return backupData;
	}
	 ...
	 */
	public void setBackupData(PtTaxType backupData) {
	...
	public void loadDataList() {
		dataList = taxTypeEJB.findAllData();
		if (dataList.isEmpty()) {
	...
	public void onRowInit(RowEditEvent<?> event) {
		...
		dataObject = (PtTaxType) event.getObject();
	...
	public boolean objectValidation(Object dataObject) {
		boolean result = true;
		PtTaxType objectToValidate;
		String message = "DATA VALIDATION";
		String messageDetail = "";

		objectToValidate = (PtTaxType) dataObject;
		...
		if (objectToValidate != null) {
			...
			if (objectToValidate.getCode() == null || objectToValidate.getCode().length() == 0) {
				messageDetail = "ERROR - The code of the tax type can not be null";
			...
			if (objectToValidate.getName() == null || objectToValidate.getName().length() == 0) {
				messageDetail = "ERROR - The name of the tax type can not be null";
				logger.error(messageDetail);
				this.createMessage(facesContext, externalContext, FacesMessage.SEVERITY_ERROR, message, messageDetail);
				result = false;
			...
			if (objectToValidate.getDescription() == null || objectToValidate.getDescription().length() == 0) {
				messageDetail = "ERROR - The description of the tax type can not be null";
				logger.error(messageDetail);
				this.createMessage(facesContext, externalContext, FacesMessage.SEVERITY_ERROR, message, messageDetail);
				result = false;
