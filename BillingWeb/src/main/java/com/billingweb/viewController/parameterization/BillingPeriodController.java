 ...
 *
 */
public class BillingPeriodController extends SimpleTableBasicClass implements Serializable, IGeneralController {

	private static final long serialVersionUID = -1559844977909164902L;

	Logger logger = (Logger) LogManager.getLogger(BillingPeriodController.class);

	String DATA_TABLE_ID = "form:" + BillingPeriodController.uiValues.getString("dataTableID");
	String NEW_PANEL_DATA_ID = "form:" + BillingPeriodController.uiValues.getString("newPanelDataID");
	 ...
	 */
	private List<PtBillingPeriod> dataList;

	/**
	 * Application level filtered data list
	 */
	private List<PtBillingPeriod> filteredDataList;
	 ...
	 */
	private PtBillingPeriod backupData;
	 ...
	 * @return the dataList
	 */
	public List<PtBillingPeriod> getDataList() {
		return dataList;
	}
	 ...
	 */
	public void setDataList(List<PtBillingPeriod> dataList) {
	 ...
	 */
	public List<PtBillingPeriod> getFilteredDataList() {
	 ...
	 */
	public void setFilteredDataList(List<PtBillingPeriod> filteredDataList) {
	 ...
	 */
	public PtBillingPeriod getBackupData() {
		return backupData;
	}
	 ...
	 */
	public void setBackupData(PtBillingPeriod backupData) {
	...
	public void init() {
		...
		if (selectedData == null) {
			selectedData = new PtBillingPeriod();
		}

		if (this.backupData == null) {
			this.backupData = new PtBillingPeriod();
		}
	...
	public void loadDataList() {
		dataList = billingPeriodEJB.findAllData();
		if (dataList.isEmpty()) {
	...
	public void pushAddNewButton() {
		this.selectedData = new PtBillingPeriod();
	...
	public boolean objectValidation(Object dataObject) {
		boolean result = true;
		PtBillingPeriod objectToValidate;
		String message = "DATA VALIDATION";
		String messageDetail = "";

		objectToValidate = (PtBillingPeriod) dataObject;
		...
		if (objectToValidate != null) {
			...
			if (objectToValidate.getName() == null || objectToValidate.getName().length() == 0) {
				messageDetail = "ERROR - The name of the billing period can not be null";
				logger.error(messageDetail);
				this.createMessage(facesContext, externalContext, FacesMessage.SEVERITY_ERROR, message, messageDetail);
				result = false;
			...
			if (objectToValidate.getDescription() == null || objectToValidate.getDescription().length() == 0) {
				messageDetail = "ERROR - The description of the billing period can not be null";
				logger.error(messageDetail);
				this.createMessage(facesContext, externalContext, FacesMessage.SEVERITY_ERROR, message, messageDetail);
				result = false;
