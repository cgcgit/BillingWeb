...
public class ConsumptionClassController extends SimpleTableBasicClass implements Serializable, IGeneralController {

	/**
	 * 
	 */
	private static final long serialVersionUID = -6645598821144056469L;
	 ...
	 */
	private List<PtConsumptionClass> dataList;

	/**
	 * Application level filtered data list
	 */
	private List<PtConsumptionClass> filteredDataList;
	 ...
	 */
	private PtConsumptionClass backupData;
	 ...
	 * @return the dataList
	 */
	public List<PtConsumptionClass> getDataList() {
		return dataList;
	}
	 ...
	 */
	public void setDataList(List<PtConsumptionClass> dataList) {
	 ...
	 */
	public List<PtConsumptionClass> getFilteredDataList() {
	 ...
	 */
	public void setFilteredDataList(List<PtConsumptionClass> filteredDataList) {
	 ...
	 */
	public PtConsumptionClass getBackupData() {
		return backupData;
	}
	 ...
	 */
	public void setBackupData(PtConsumptionClass backupData) {
	...
	public void init() {
		...
		if (selectedData == null) {
			selectedData = new PtConsumptionClass();
		}

		if (this.backupData == null) {
			this.backupData = new PtConsumptionClass();
		}
	...
	public void loadDataList() {
		dataList = consumptionClassEJB.findAllData();
		if (dataList.isEmpty()) {
	...
	public void onRowInit(RowEditEvent<?> event) {
		...
		dataObject = (PtConsumptionClass) event.getObject();
	...
	public void pushConfirmButtonAddNewDialog() {
		...
		try {
			if (objectValidation(this.selectedData)) {
				consumptionClassEJB.insertData(this.selectedData);
	...
	public void pushConfirmButtonDeleteDialog() {
		...
		try {
			if (this.selectedData != null) {
				consumptionClassEJB.deleteData(this.selectedData);
	...
	public boolean objectValidation(Object dataObject) {
		boolean result = true;
		PtConsumptionClass objectToValidate;
		String message = "DATA VALIDATION";
		String messageDetail = "";

		objectToValidate = (PtConsumptionClass) dataObject;
		...
		if (objectToValidate != null) {
			...
			if (objectToValidate.getCode() == null || objectToValidate.getCode().length() == 0) {
				messageDetail = "ERROR - The code of the consumption class can not be null";
			...
			if (objectToValidate.getName() == null || objectToValidate.getName().length() == 0) {
				messageDetail = "ERROR - The name of the consumption class can not be null";
				logger.error(messageDetail);
				this.createMessage(facesContext, externalContext, FacesMessage.SEVERITY_ERROR, message, messageDetail);
				result = false;
			...
			if (objectToValidate.getDescription() == null || objectToValidate.getDescription().length() == 0) {
				messageDetail = "ERROR - The description of the consumption class can not be null";
				logger.error(messageDetail);
				this.createMessage(facesContext, externalContext, FacesMessage.SEVERITY_ERROR, message, messageDetail);
				result = false;
