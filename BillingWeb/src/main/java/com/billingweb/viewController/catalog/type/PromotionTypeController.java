 ...
 *
 */
public class PromotionTypeController extends SimpleTableBasicClassWithLists implements Serializable, IGeneralController {

	/**
	 * 
	 */
	private static final long serialVersionUID = 5921831542172836029L;

	Logger logger = (Logger) LogManager.getLogger(PromotionTypeController.class);

	String DATA_TABLE_ID = "form:" + PromotionTypeController.uiValues.getString("dataTableID");
	String NEW_PANEL_DATA_ID = "form:" + PromotionTypeController.uiValues.getString("newPanelDataID");
	 ...
	 * Entity data list
	 */
	private List<CtPromotionType> dataList;

	/**
	 * Application level filtered data list
	 */
	private List<CtPromotionType> filteredDataList;

	/**
	 * Selected data row in the table
	 */
	@Inject
	private CtPromotionType selectedData;

	/**
	 * Backup object for entity type(delete/modify operations)
	 */
	private CtPromotionType backupData;
	 ...
	 * @return the dataList
	 */
	public List<CtPromotionType> getDataList() {
		return dataList;
	}
	 ...
	 */
	public void setDataList(List<CtPromotionType> dataList) {
	 ...
	 */
	public List<CtPromotionType> getFilteredDataList() {
	 ...
	 */
	public void setFilteredDataList(List<CtPromotionType> filteredDataList) {
	 ...
	 */
	public CtPromotionType getSelectedData() {
		return selectedData;
	}
	 ...
	 */
	public void setSelectedData(CtPromotionType selectedData) {
	 ...
	 */
	public CtPromotionType getBackupData() {
		return backupData;
	}
	 ...
	 */
	public void setBackupData(CtPromotionType backupData) {
	...
	public void init() {
		...
		if (selectedData == null) {
			selectedData = new CtPromotionType();
		}

		if (this.backupData == null) {
			this.backupData = new CtPromotionType();
		}
	...
	public void loadDataList() {
		dataList = promotionTypeEJB.findAllData();
		if (dataList.isEmpty()) {
	...
	public void onRowInit(RowEditEvent<?> event) {
		...
		messageDetail = "Editing promotion type: " + dataObject.getCode();
	...
	public void onRowEdit(RowEditEvent<?> event) throws ValidatorException {
		...
		try {
			...
			if (this.objectValidation(dataObject)) {
				...
				logger.info("Update promotion type: " + this.selectedData.toString() + " - " +messageDetail);
				this.createMessage(facesContext, externalContext, FacesMessage.SEVERITY_INFO, message, messageDetail);
	...
	public void onRowCancel(RowEditEvent<?> event) {
		...
		messageDetail = "Cancelled the edition of the promotion type";
		logger.info(messageDetail);
		this.createMessage(facesContext, externalContext, FacesMessage.SEVERITY_INFO, message, messageDetail);
	...
	public void pushAddNewButton() {
		this.selectedData = new CtPromotionType();
	...
	public void pushConfirmButtonAddNewDialog() {
		...
		try {
			if (objectValidation(this.selectedData)) {
				...
				promotionTypeEJB.insertData(this.selectedData);
				messageDetail = "Data saves succesfully";
				logger.info("Create promotion type: " + this.selectedData.toString() + " - " +messageDetail);
				this.createMessage(facesContext, externalContext, FacesMessage.SEVERITY_INFO, message, messageDetail);
	...
	public void pushConfirmButtonDeleteDialog() {
		...
		try {
			if (this.selectedData != null) {
				promotionTypeEJB.deleteData(this.selectedData);
				messageDetail = "Data deletes succesfully";
				logger.info("Delete promotion type: " + this.selectedData.toString() + " - " + messageDetail);
				this.createMessage(facesContext, externalContext, FacesMessage.SEVERITY_INFO, message, messageDetail);
			} else {
				error = true;
