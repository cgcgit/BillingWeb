...
import static com.billingweb.model.Tables.VW_PRODUCT_FEE_TYPE;
import static com.billingweb.model.Tables.CT_FEE_TYPE;
import static com.billingweb.model.Tables.CT_PROD_FEE_TYPE;
...
import com.billingweb.model.tables.daos.CtProdFeeTypeDao;
...
public class ProductFeeTypeEJB implements ProductFeeTypeEJBLocal {
	...
	public List<CtFeeType> findEntityTypeCandidates(Integer parentId) throws BillingWebDataAccessException {
		...
		com.billingweb.model.tables.CtProdFeeType pft = CT_PROD_FEE_TYPE.as("pft");
	...
	public List<VwProductFeeType> findRelatedEntityTypesView(Integer parentId) throws BillingWebDataAccessException {
		...
		try {
			result = create.select()
					.from(VW_PRODUCT_FEE_TYPE)
					.where(VW_PRODUCT_FEE_TYPE.PRODUCT_TYPE_ID.eq(val(parentId)))
					.orderBy(VW_PRODUCT_FEE_TYPE.FEE_TYPE_CODE)
	...
	public CtProdFeeType findEntityRelationType(Integer entityRelationTypeId) throws BillingWebDataAccessException {
		...
		try {
			result = create.selectFrom(CT_PROD_FEE_TYPE)
					.where(CT_PROD_FEE_TYPE.PROD_FEE_TYPE_ID.eq(val(entityRelationTypeId)))
					.fetch()
					.into(CtProdFeeType.class);
			
			if (result.size()>1) {
				errorMessage = "Error while try to find the product fee type for the product_fee_type_id : " + entityRelationTypeId
			...
			errorMessage = "Error while try to find the product fee type for the product_fee_type_id : " + entityRelationTypeId  + " - " + e.getMessage();
			logger.error(errorMessage);
			throw new BillingWebDataAccessException(errorMessage, e);
		}
	}
	...
	@Override
	public CtProdFeeType findEntityRelationType(Integer parentId, Integer childId)
		...
		try {
			result = create.selectFrom(CT_PROD_FEE_TYPE)
					.where(CT_PROD_FEE_TYPE.PRODUCT_TYPE_ID.eq(val(parentId))).
					and(CT_PROD_FEE_TYPE.FEE_TYPE_ID.eq(val(childId)))					
					.fetch()
					.into(CtProdFeeType.class);
	...
	public VwProductFeeType findEntityRelationTypeView(Integer parentId, Integer childId)
		...
		try {
			result = create.selectFrom(VW_PRODUCT_FEE_TYPE)
					.where(VW_PRODUCT_FEE_TYPE.PRODUCT_TYPE_ID.eq(val(parentId))).
					and(VW_PRODUCT_FEE_TYPE.FEE_TYPE_ID.eq(val(childId)))
					.orderBy(VW_PRODUCT_FEE_TYPE.PROD_FEE_TYPE_ID)
					.fetch()
	...
	public void insertData(CtProdFeeType dataObject) throws BillingWebDataAccessException {
		...
		try {			
			...
			errorMessage = "Error inserting the product fee type object (value: " + dataObject.toString() + ") - " + e.getMessage();
			logger.error(errorMessage);
			throw new BillingWebDataAccessException(errorMessage, e);
		}
	...
	public void updateData(CtProdFeeType dataObject) throws BillingWebDataAccessException {
		...
		try {			
			...
			errorMessage = "Error updating the product fee type object (value: " + dataObject.toString() + ") - " + e.getMessage();
			logger.error(errorMessage);
			throw new BillingWebDataAccessException(errorMessage, e);
		}
	...
	public void deleteData(CtProdFeeType dataObject) throws BillingWebDataAccessException {
		...
		try {
			...
			errorMessage = "Error deleting the product fee type object (value: "
